...
@router.put("/{task_id}", response_model=schemas.TaskOut, summary="Get a single task by its ID")
async def update_task(task_id: int,
                      payload: schemas.TaskUpdate,
                      user=Depends(deps.get_current_user),
                      db: AsyncSession = Depends(deps.get_db)):
    task: models.Task = await db.get(models.Task, task_id)
...
@router.get("", response_model=list[schemas.TaskOut], summary="List all tasks for the current user")
async def list_tasks(status: schemas.TaskStatus | None = None,
...
async def search(q: str,
    ...
    stmt = select(models.Task).where(
        ...
        or_(models.Task.title.ilike(f"%{q}%"),
